...
fn fetch_tokenmix_models(api_key: &str) -> ModelsResult {
    ...
    let models: Vec<ModelInfo> = json["data"]
        ...
        .iter()
        .map(|m| {
            // Try common pricing field locations used by relay platforms
...
fn fetch_claude_models() -> ModelsResult {
    // Claude doesn't have a public model list API that works without special access.
    // Return the known available models with published pricing (per 1M tokens in USD).
    let models = vec![
