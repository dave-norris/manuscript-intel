...
fn run_competition_analysis(app: &AppHandle, req: &CompetitionRequest) -> CompetitionResult {
    ...
    if !analysis_dir.exists() {
        return err("No _analysis folder. Run Full Analysis and Optimize Keywords first.");
    }

    // ── Step 1: Load keywords ─────────────────────────────────────────────────
    let keywords = load_keywords(&analysis_dir);
    if keywords.is_empty() {
        return err("No keywords found. Run Optimize Keywords first.");
    }
...
fn load_keywords(analysis_dir: &Path) -> Vec<String> {
    // Try kdp-keywords.md first, fall back to genre-data.json
    if let Ok(text) = fs::read_to_string(analysis_dir.join("kdp-keywords.md")) {
        let keywords: Vec<String> = text.lines()
            .filter_map(|line| {
                let t = line.trim();
                // Lines like: **1. `christian historical fiction rome`**
                if t.contains('`') {
                    let start = t.find('`')?  + 1;
                    let end   = t.rfind('`')?;
                    if end > start {
                        let kw = t[start..end].trim().to_string();
                        if !kw.is_empty() { return Some(kw); }
                    }
                }
                None
            })
            .collect();
        if !keywords.is_empty() { return keywords; }
    }

    // Fall back to genre signals from genre-data.json
    if let Ok(text) = fs::read_to_string(analysis_dir.join("genre-data.json")) {
        if let Ok(v) = serde_json::from_str::<Value>(&text) {
            if let Some(ebook) = v["kdp"]["ebook"].as_array() {
                return ebook.iter()
                    .filter_map(|x| x.as_str())
                    .map(|s| s.split('>').last().unwrap_or(s).trim().to_lowercase())
                    .collect();
            }
        }
    }
